...
class DDQNAgent:
    def __init__(self, q_net, device="cpu", gamma=0.99, lr=5e-4, epsilon=1.0, epsilon_decay=0.998, epsilon_min=0.05):
        ...
        self.target_net = type(q_net)().to(device)
    ...
    def update(self, batch_size):
        ...
        with torch.no_grad():
            next_actions = self.q_net(ns_img, ns_goal).argmax(dim=1, keepdim=True)
            next_q = self.target_net(ns_img, ns_goal).gather(1, next_actions).squeeze()
